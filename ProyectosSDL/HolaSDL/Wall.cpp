...
bool Wall::collide(const Ball *object, Vector2D &collisionPosition, Vector2D &reflection)
{
	if (object->position().isIn(
		position.getX() - object->getRadius(),
		position.getY(),
	...
	{
		if ((object->position().getY() - position.getY()) * (width) - (object->position().getX() - position.getX()) * (height) < 0.0)
		{
			if ((object->position().getY() - position.getY() - height) * (width) - (object->position().getX() - position.getX()) * (-height) < 0.0)
			{
